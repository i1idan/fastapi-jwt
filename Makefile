...
fmt:              ## Format code using black & isort.
	$(ENV_PREFIX)isort fastapi_jwt/
	$(ENV_PREFIX)black -l 79 fastapi_jwt/
...
test: lint        ## Run tests and generate coverage report.
	$(ENV_PREFIX)pytest -v --cov-config .coveragerc --cov=fastapi_jwt -l --tb=short --maxfail=1 tests/
...
release:          ## Create a new tag for release.
	...
	@git tag $${TAG}
	@echo "$${TAG}" > fastapi_jwt/VERSION
	@$(ENV_PREFIX)gitchangelog > HISTORY.md
	@git add fastapi_jwt/VERSION HISTORY.md
...
switch-to-poetry: ## Switch to poetry package manager.
	...
	@echo "fastapi_jwt = 'fastapi_jwt.__main__:main'" >> pyproject.toml
...
shell:            ## Open a shell in the project.
	...
	@./.venv/bin/ipython -c "from fastapi_jwt import *"

.PHONY: docker-build
docker-build:	  ## Builder docker images
	@docker-compose -f docker-compose-dev.yaml -p fastapi_jwt build

.PHONY: docker-run
docker-run:  	  ## Run docker development images
	@docker-compose -f docker-compose-dev.yaml -p fastapi_jwt up -d

.PHONY: docker-stop
docker-stop: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p fastapi_jwt down

.PHONY: docker-ps
docker-ps: 	  ## Bring down docker dev environment
	@docker-compose -f docker-compose-dev.yaml -p fastapi_jwt ps
